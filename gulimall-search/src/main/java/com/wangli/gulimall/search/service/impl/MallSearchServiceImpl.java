...
public class MallSearchServiceImpl implements MallSearchService {
    ...
    public SearchResult search(SearchParam param) {
        ...
        try {
            ...
            log.debug("es查询到的数据：{}", response);

            //3、分析响应数据，封装成我们需要的格式
            result = buildSearchResult(response, param);
        } catch (IOException e) {
            log.error("MallSearchServiceImpl.search error:", e);
        }
        log.debug("封装好的es返回数据：{}", JSONUtil.toJsonStr(result));
        return result;
    }
    ...
    private SearchRequest buildSearchRequest(SearchParam param) {
        ...
        if (param.getAttrs() != null && !param.getAttrs().isEmpty()) {
            ...
            param.getAttrs().forEach(item -> {
                ...
                //每一个不同查询属性都需要构建一个NestedQueryBuilder
...
//            SortOrder sortOrder = "asc".equalsIgnoreCase(sortFileds[1]) ? SortOrder.ASC : SortOrder.DESC;
            ...
            searchSourceBuilder.sort(sortFileds[0], SortOrder.fromString(sortFileds[1]));
        }
        ...
        //3.3.2、 在每个属性ID下，按照属性名进行聚合
        attrIdAgg.subAggregation(AggregationBuilders.terms("attr_name_agg").field("attrs.attrName").size(1));
        //3.3.3、 在每个属性ID下，按照属性值进行聚合
